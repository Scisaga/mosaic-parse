...
from app.utils.ids import new_document_id, new_job_id
...
class JobService:
    ...
    async def start(self) -> None:
        ...
        for orphan in await self.repository.list(status=JobStatus.QUEUED, limit=10_000):
            await self.repository.fail(orphan.id, "server_restarted", "server restarted before the queued job began")
        self._workers = [
    ...
    @staticmethod
    def _validate_selection(options: DocumentParseOptions, page_count: int) -> list[int]:
        try:
    ...
    async def parse_sync(
        ...
        *,
        ...
        options: DocumentParseOptions | dict[str, object] | None = None,
    ) -> DocumentParseResult:
        parsed_options = options if isinstance(options, DocumentParseOptions) else DocumentParseOptions.model_validate(options or {})
        try:
        ...
        try:
            ...
            pages = self._validate_selection(parsed_options, source.page_count)
            sync_max_bytes = int(setting(self.settings, "sync_max_bytes", 20 * 1024 * 1024))
            sync_max_pages = int(setting(self.settings, "sync_max_pages", 10))
            if source.size_bytes > sync_max_bytes or len(pages) > sync_max_pages:
                raise ServiceError(
                    "sync_limit_exceeded",
                    "document exceeds synchronous limits; use the asynchronous jobs endpoint",
                    status_code=409,
                    details={
            ...
            return await self.parser_service.parse(source, parsed_options, document_id=identifier)
        finally:
            try:
                await self.storage.delete_job(identifier)
            finally:
    ...
    async def _enqueue_source(
        ...
        *,
        ...
        pages = self._validate_selection(options, source.page_count)
        record = JobRecord(
            id=job_id,
            ...
            options=options,
            progress=JobProgress(current=0, total=len(pages)),
    ...
    async def _run_job(self, job_id: str) -> None:
        ...
        try:
            ...
            selected_pages = parse_page_range(record.options.page_range, record.page_count)
            ...
            async def progress(current: int, total: int, state: str) -> None:
                nonlocal live_progress_current
                ...
                if terminal_page or state == "document.started":
                    await self.repository.update_progress(job_id, current, total)
                event_name = state if state in {"page.completed", "page.warning", "page.failed"} else "job.progress"
                if postprocess:
            ...
            await self.repository.update_progress(job_id, record.progress.total, record.progress.total)
            partial = result.route_summary.failed_pages > 0
            ...
            await self._emit(
                ...
                failed_pages=result.route_summary.failed_pages,
            )
        except asyncio.CancelledError:
            ...
            if current and not current.is_terminal:
                await self.repository.fail(job_id, "parse_failed", "document parsing failed")
                await self._emit(job_id, "job.failed", code="parse_failed", message=type(exc).__name__)
    ...
    async def retry_job(self, job_id: str, *, page_range: str | None = None) -> JobRecord:
        if not self._started:
            await self.start()
        original = await self.get_job(job_id)
        if original.status not in {JobStatus.FAILED, JobStatus.PARTIAL, JobStatus.CANCELLED}:
            raise ServiceError("job_not_retryable", "only failed, partial, or cancelled jobs can be retried", status_code=409)
        options = original.options.model_copy(update={"page_range": page_range}) if page_range is not None else original.options.model_copy()
        new_id = new_job_id()
        ...
        if not original_source.path.is_file():
            raise ServiceError("source_expired", "the original source file is no longer available", status_code=409)
        self._acquire_async_token()
        admission_owned = True
        try:
    ...
    async def cancel_job(self, job_id: str) -> JobRecord:
        ...
        if record.is_terminal:
            if record.status == JobStatus.CANCELLED:
                return record
            raise ServiceError("job_not_cancellable", "completed jobs cannot be cancelled", status_code=409)
        ...
        try:
            ...
            if latest.status == JobStatus.CANCELLED:
                return latest
            raise ServiceError("job_not_cancellable", "job finished before cancellation", status_code=409) from exc
        await self._emit(job_id, "job.cancelled")
        return cancelled
    ...
    async def delete_job(self, job_id: str) -> bool:
        ...
        if not record.is_terminal:
            raise ServiceError("job_active", "cancel an active job before deleting it", status_code=409)
        await self.storage.delete_job(job_id)
    ...
    async def get_result(self, job_id: str, output_format: str = "markdown") -> str:
        record = await self.get_job(job_id)
        ...
        try:
            return await self.storage.read_result(job_id, output_format)
        except FileNotFoundError as exc:
            raise ServiceError("result_missing", "persisted result file is missing", status_code=500) from exc

    async def events(self, job_id: str, last_event_id: int | None = None) -> AsyncIterator[JobEvent]:
