    ...
    <form className="options-panel" onSubmit={submit} aria-labelledby="options-heading">
      <div className="options-title-row">
        <div className="options-title section-heading-copy">
          ...
          <p>服务自动选择文档、视觉或视频链路，并生成统一的 GFM Markdown。</p>
        </div>
          ...
          <div ref={advancedDialogRef} id="advanced-options-dialog" className="advanced-popover" role="dialog" aria-modal="true" aria-labelledby="advanced-options-heading" aria-describedby="advanced-options-description">
            <div className="advanced-popover-header">
              <div>
                ...
                <p id="advanced-options-description">调整扫描页、OCR、图片按需描述和解析超时。</p>
              </div>
            ...
            <div className="advanced-options">
              <label>
                <span>扫描页处理</span>
                <select aria-label="扫描页处理" data-advanced-initial value={options.scanPolicy} onChange={(event) => update('scanPolicy', event.target.value as ParseOptions['scanPolicy'])} disabled={busy}>
                  <option value="auto">自动处理（推荐）</option>
                  <option value="skip">跳过并保留页面图像</option>
                </select>
                <small>自动模式仅增强检测出的扫描页和复杂视觉表格。</small>
              </label>
              <label>
                <span>OCR 语言</span>
                <input value={options.language} onChange={(event) => update('language', event.target.value)} placeholder="zh,en" disabled={busy} />
              </label>
              <label>
                <span>嵌入图片描述</span>
                <select value={options.describeImages ? 'on' : 'off'} onChange={(event) => update('describeImages', event.target.value === 'on')} disabled={busy}>
                  <option value="off">仅保留资产（推荐）</option>
                  <option value="on">生成模型描述</option>
                </select>
              </label>
              <label>
                <span>图片描述语言</span>
                <select value={options.descriptionLanguage} onChange={(event) => update('descriptionLanguage', event.target.value as ParseOptions['descriptionLanguage'])} disabled={busy || !options.describeImages}>
