...
function sourceType(source: SourceSelection): 'pdf' | 'image' | 'tiff' | 'unknown' {
  ...
  if (type.startsWith('image/') || /\.(png|jpe?g|webp)$/.test(path)) return 'image'
  return 'unknown'
}
    ...
    <article className="document-preview panel-surface" aria-labelledby="preview-heading">
      <div className="panel-header">
        <div className="panel-heading-copy">
          <span className="eyebrow">03 · 原始文档</span>
          <h2 id="preview-heading">文档预览</h2>
          <p title={sourceName || undefined}>{sourceName || '等待选择文档'}</p>
        </div>
        {pdf && (
      ...
      <div className="preview-stage">
        {empty && (
          <WorkspaceState variant="input-empty" title="预览区等待文档" description="上传 PDF 或图片后，可在这里检查原始页面。" />
        )}
        ...
        {sourceUrl && kind === 'unknown' && (
          <WorkspaceState variant="error" title="不支持预览该格式" description="请选择 PDF、PNG、JPEG、WEBP 或 TIFF。" role="alert" live="assertive" />
        )}
      </div>
    </article>
  )
